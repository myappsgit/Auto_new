...
public class Page {
	    ...
	    public static boolean clickButton(WebDriver driver, String pageName, String buttonName) throws InterruptedException {
			...
			if(buttonPath==null){
				fileObj = UtilFunctions.getJSONFileObjBasedOnPageName("");
				elementType = UtilFunctions.getElementStringAndType(fileObj, "BUTTONS." + buttonName);
				buttonPath = elementType[0];
				buttonMethod = elementType[1];
			}
	    ...
	    public static boolean setTextBox(WebDriver driver, String pageName, String value, String fieldName){
	        try {
	            ...
	            if (txtObj == null) {
	            	...
	            	try{
//						((JavascriptExecutor) Hooks.getDriver()).executeScript("arguments[0].value='"+value+"'", txtObj);
						txtObj.click();
						txtObj.clear();
...
//	            linkElement.click();
	    ...
	    public static WebElement findPage(WebDriver driver, String tabName, String pageName, int waitTime){
	        ...
	        if(pageName.equals("Consumer")){
	        	SeleniumFunctions.explicitWait(Hooks.getDriver(),10,"",By.tagName("consumer"));
	        	elt = SeleniumFunctions.findElement(driver, By.tagName("consumer"));
	        }
			else if(pageName.equals("ServiceProvider")){
				SeleniumFunctions.explicitWait(Hooks.getDriver(),10,"",By.tagName("service-provider-header"));
				elt = SeleniumFunctions.findElement(driver, By.tagName("service-provider-header"));
			}
	        else {
